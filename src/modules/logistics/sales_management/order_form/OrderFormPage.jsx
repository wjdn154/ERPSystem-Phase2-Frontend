...
const OrderFormPage = ({initialData}) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiClient.post(LOGISTICS_API.ORDER_LIST_API, searchParams);
            const data = response.data;
            setSearchData(data);
            console.log(data)

            notify('success', '조회 성공', '주문서 조회 성공.', 'bottomRight');
        } catch (error) {
            notify('error', '조회 오류', '주문서 조회 중 오류가 발생했습니다.', 'top');
        }
    };
        ...
        <Box sx={{ margin: '20px' }}>
            <Grid container spacing={3}>
                <Grid item xs={12} md={12}>
                    <WelcomeSection
                        ...
                        description={(
                            <Typography>
                                주문서 페이지는 <span>고객이 주문서에 따라 실제 주문을 넣을 때 사용하는 문서</span>를 관리함. 이 페이지에서는 <span>주문서를 생성, 수정, 삭제</span>할 수 있으며, 주문서는 <span>주문서와 연동</span>되어 자동으로 항목을 가져오거나 수동으로 추가할 수 있음. 각 주문서에는 <span>주문 날짜, 납기일, 결제 조건</span> 등 세부 사항을 입력할 수 있음.
                            </Typography>
                        )}
                        tabItems={tabItems()}
                ...
                <Grid sx={{ padding: '0px 20px 0px 20px' }} container spacing={3}>
                    <Grid item xs={12} md={12} sx={{ minWidth: '1000px !important', maxWidth: '1500px !important' }}>
                        <Grow in={true} timeout={200}>
                            <Paper elevation={3} sx={{ height: '100%'}}>
                                <Typography variant="h6" sx={{ padding: '20px' }}>주문서 목록</Typography>
                                    ...
                                    <Table
                                        ...
                                        onRow={(record) => ({
                                            ...
                                            onClick: async () => {
                                                ...
                                                const id = record.id;
                                                try {
                                                    const response = await apiClient.post(LOGISTICS_API.ORDER_DETAIL_API(id));
                                                    setDetailOrder(response.data);
                                                    console.log('detailOrder: ', response.data)
                                                    setOrdersDetails(detailOrder)
                                                    setEditOrder(true);

                                                    notify('success', '주문 조회', '주문 정보 조회 성공.', 'bottomRight')
                                                } catch (error) {
                                                    notify('error', '조회 오류', '데이터 조회 중 오류가 발생했습니다.', 'top');
                                                }
                                            },
                    ...
                    {editOrder && (
                        <Grid item xs={12} md={12} sx={{ minWidth: '1000px !important', maxWidth: '1500px !important' }}>
                            <Grow in={true} timeout={200}>
                                <Paper elevation={3} sx={{ height: '100%' }}>
                                    <Typography variant="h6" sx={{ padding: '20px' }}>주문 상세정보 및 수정</Typography>
                                    <Grid sx={{ padding: '0px 20px 0px 20px' }}>
                                        <Form
                                                ...
                                                <Col span={6}>
                                                    <Form.Item name="journalEntry">
                                                        <Space.Compact>
                                                            ...
                                                            <Select
                                                                style={{ width: '70%' }}
                                                                value={orderParam.journalEntryCode}
                                                                onChange={(value) => {
                                                                    setOrderParam((prevState) => ({
                                                                        ...prevState,
            ...
            {activeTabKey === '2' && (
                <Grid item xs={12} md={12} sx={{ minWidth: '1000px !important', maxWidth: '1500px !important' }}>
                    <Grow in={true} timeout={200}>
                        <Paper elevation={3} sx={{ height: '100%' }}>
                            <Typography variant="h6" sx={{ padding: '20px' }}>주문서 등록</Typography>
                            <Grid sx={{ padding: '0px 20px 0px 20px' }}>
                                <Form
                                    initialValues={detailOrder}
                                    form={form}
